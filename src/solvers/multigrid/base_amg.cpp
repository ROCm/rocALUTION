...
void BaseAMG<OperatorType, VectorType, ValueType>::BuildHierarchy(void) {
  ...
  if (this->hierarchy_ == false) {
    ...
    while(op_list_.back()->get_nrow() > (IndexType2) this->coarse_size_) {
      ...
      if (this->levels_ > 19)
        LOG_VERBOSE_INFO(2, "*** warning: BaseAMG::Build() Current number of levels: " << this->levels_);
...
#ifdef SUPPORT_COMPLEX
template class BaseAMG< GlobalMatrix<std::complex<double> >, GlobalVector<std::complex<double> >, std::complex<double> >;
