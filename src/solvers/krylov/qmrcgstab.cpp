...
void QMRCGStab<OperatorType, VectorType, ValueType>::Build(void) {
  ...
  this->d_.Allocate("d", this->op_->get_nrow());
...
void QMRCGStab<OperatorType, VectorType, ValueType>::SolveNonPrecond_(const VectorType &rhs,
  ...
  c = ValueType(1.0) / sqrt(ValueType(1.0) + theta1sq);
  ...
  c = ValueType(1.0) / sqrt(ValueType(1.0) + theta2sq);
  ...
  while (!this->iter_ctrl_.CheckResidual(res_norm, this->index_)) {
    ...
    // p = r + beta * (p - omega * v)
    p->AddScale(*v, ValueType(-1.0)*omega);
    p->Scale(beta);
    p->AddScale(*r, ValueType(1.0));
...
template class QMRCGStab< LocalMatrix<std::complex<double> >, LocalVector<std::complex<double> >, std::complex<double> >;
template class QMRCGStab< LocalMatrix<std::complex<float> >,  LocalVector<std::complex<float> >, std::complex<float> >;
#endif
...
#ifdef SUPPORT_COMPLEX
template class QMRCGStab< GlobalMatrix<std::complex<double> >, GlobalVector<std::complex<double> >, std::complex<double> >;
template class QMRCGStab< GlobalMatrix<std::complex<float> >,  GlobalVector<std::complex<float> >, std::complex<float> >;
#endif
