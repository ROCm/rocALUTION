...
void FCG<OperatorType, VectorType, ValueType>::SolveNonPrecond_(const VectorType &rhs,
  ...
  // r = r - alpha/rho * q
  r->AddScale(*q, ValueType(-1.0)*alpha/rho);
  ...
  while (!this->iter_ctrl_.CheckResidual(rocalution_abs(res), this->index_)) {
    ...
    // gamma = (r,q)
    gamma = r->Dot(*q);
    gamma_rho = ValueType(-1.0) * gamma / rho;
...
template class FCG< LocalMatrix<std::complex<double> >,  LocalVector<std::complex<double> >, std::complex<double> >;
template class FCG< LocalMatrix<std::complex<float> >,  LocalVector<std::complex<float> >, std::complex<float> >;
#endif
...
#ifdef SUPPORT_COMPLEX
template class FCG< GlobalMatrix<std::complex<double> >,  GlobalVector<std::complex<double> >, std::complex<double> >;
template class FCG< GlobalMatrix<std::complex<float> >,  GlobalVector<std::complex<float> >, std::complex<float> >;
#endif
