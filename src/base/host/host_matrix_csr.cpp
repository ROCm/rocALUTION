...
// ----------------------------------------------------------
template <typename ValueType>
bool HostMatrixCSR<ValueType>::RugeStueben(const ValueType eps, BaseMatrix<ValueType> *prolong,
                                                                BaseMatrix<ValueType> *restrict) const {
...
#pragma omp parallel for schedule(dynamic, 1024)
  ...
  cast_prolong->mat_.col = (int*) realloc(cast_prolong->mat_.col, cast_prolong->mat_.row_offset[this->nrow_]*sizeof(int));
  cast_prolong->mat_.val = (ValueType*) realloc(cast_prolong->mat_.val, cast_prolong->mat_.row_offset[this->nrow_]*sizeof(ValueType));
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::InitialPairwiseAggregation(const ValueType beta, int &nc, BaseVector<int> *G, int &Gsize,
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::InitialPairwiseAggregation(const BaseMatrix<ValueType> &mat, const ValueType beta,
                                                          int &nc, BaseVector<int> *G, int &Gsize, int **rG,
  ...
  for (int k=0; k<this->nrow_; ++k) {
    ...
    if (min_j != -1) {
      ...
      for (int j=this->mat_.row_offset[i]; j<this->mat_.row_offset[i+1]; ++j) {
        ...
        if (cast_G->vec_[col_j] != -2) continue;
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::FurtherPairwiseAggregation(const ValueType beta, int &nc, BaseVector<int> *G, int &Gsize,
                                                          int **rG, int &rGsize, const int ordering) const {
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::FurtherPairwiseAggregation(const BaseMatrix<ValueType> &mat, const ValueType beta,
                                                          int &nc, BaseVector<int> *G, int &Gsize, int **rG,
                                                          int &rGsize, const int ordering) const {
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::CoarsenOperator(BaseMatrix<ValueType> *Ac, const int nrow, const int ncol, const BaseVector<int> &G,
...
template <typename ValueType>
bool HostMatrixCSR<ValueType>::Key(long int &row_key,
                                   long int &col_key,
