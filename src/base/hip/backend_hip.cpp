  ...
  if (num_dev < 1) {
    ...
    for (int idev=0; idev<num_dev; idev++) {
      ...
      if (hip_status_t == hipSuccess) {
        ...
        if ((rocblas_create_handle(static_cast<rocblas_handle*>(_get_backend_descriptor()->ROC_blas_handle)) == rocblas_status_success) &&
            (rocsparse_create_handle(static_cast<rocsparse_handle*>(_get_backend_descriptor()->ROC_sparse_handle)) == rocsparse_status_success)) {
...
void rocalution_stop_hip(void) {
  ...
  if (_get_backend_descriptor()->accelerator) {
    ...
    if (rocblas_destroy_handle(*(static_cast<rocblas_handle*>(_get_backend_descriptor()->ROC_blas_handle))) != rocblas_status_success) {
      LOG_INFO("Error in rocblas_destroy_handle");
    }

    if (rocsparse_destroy_handle(*(static_cast<rocsparse_handle*>(_get_backend_descriptor()->ROC_sparse_handle))) != rocsparse_status_success) {
