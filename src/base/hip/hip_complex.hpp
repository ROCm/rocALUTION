...
__device__ static __inline__ double hip_pow(const double val, const double exp) { return pow(val, exp); }
__device__ static __inline__ float hip_pow(const float val, const double exp) { return powf(val, exp); }
