...
void GlobalMatrix<ValueType>::AllocateCSR(const std::string name, const int local_nnz, const int ghost_nnz) {
  ...
  this->matrix_interior_.AllocateCSR(interior_name, local_nnz, this->pm_->GetLocalSize(), this->pm_->GetLocalSize());
  this->matrix_ghost_.AllocateCSR(ghost_name, ghost_nnz, this->pm_->GetLocalSize(), this->pm_->GetNumReceivers());
...
void GlobalMatrix<ValueType>::AllocateCOO(const std::string name, const int local_nnz, const int ghost_nnz) {
  ...
  this->matrix_interior_.AllocateCOO(interior_name, local_nnz, this->pm_->GetLocalSize(), this->pm_->GetLocalSize());
  this->matrix_ghost_.AllocateCOO(ghost_name, ghost_nnz, this->pm_->GetLocalSize(), this->pm_->GetNumReceivers());
...
template <typename ValueType>
void GlobalMatrix<ValueType>::SetDataPtrCSR(int **local_row_offset, int **local_col, ValueType **local_val,
  ...
  this->matrix_interior_.SetDataPtrCSR(local_row_offset, local_col, local_val, interior_name,
                                       local_nnz, this->pm_->GetLocalSize(), this->pm_->GetLocalSize());
  this->matrix_ghost_.SetDataPtrCSR(ghost_row_offset, ghost_col, ghost_val, ghost_name,
                                    ghost_nnz, this->pm_->GetLocalSize(), this->pm_->GetNumReceivers());
...
template <typename ValueType>
void GlobalMatrix<ValueType>::SetDataPtrCOO(int **local_row, int **local_col, ValueType **local_val,
  ...
  this->matrix_interior_.SetDataPtrCOO(local_row, local_col, local_val, interior_name,
                                       local_nnz, this->pm_->GetLocalSize(), this->pm_->GetLocalSize());
  this->matrix_ghost_.SetDataPtrCOO(ghost_row, ghost_col, ghost_val, ghost_name,
                                    ghost_nnz, this->pm_->GetLocalSize(), this->pm_->GetNumReceivers());
...
void GlobalMatrix<ValueType>::SetLocalDataPtrCSR(int **row_offset, int **col, ValueType **val,
  ...
  this->matrix_interior_.SetDataPtrCSR(row_offset, col, val, interior_name, nnz,
                                       this->pm_->GetLocalSize(), this->pm_->GetLocalSize());
...
void GlobalMatrix<ValueType>::SetLocalDataPtrCOO(int **row, int **col, ValueType **val,
  ...
  this->matrix_interior_.SetDataPtrCOO(row, col, val, interior_name, nnz,
                                       this->pm_->GetLocalSize(), this->pm_->GetLocalSize());
...
void GlobalMatrix<ValueType>::SetGhostDataPtrCSR(int **row_offset, int **col, ValueType **val,
  ...
  this->matrix_ghost_.SetDataPtrCSR(row_offset, col, val, ghost_name, nnz,
...
void GlobalMatrix<ValueType>::ReadFileMTX(const std::string filename) {
  ...
  interior_name.erase(remove_if(interior_name.begin(), interior_name.end(), isspace), interior_name.end());
...
void GlobalMatrix<ValueType>::InitialPairwiseAggregation(const ValueType beta, int &nc, LocalVector<int> *G, int &Gsize,
  ...
  this->matrix_interior_.InitialPairwiseAggregation(tmp, beta, nc, G, Gsize, rG, rGsize, ordering);
...
void GlobalMatrix<ValueType>::FurtherPairwiseAggregation(const ValueType beta, int &nc, LocalVector<int> *G, int &Gsize,
  ...
  this->matrix_interior_.FurtherPairwiseAggregation(tmp, beta, nc, G, Gsize, rG, rGsize, ordering);
...
void GlobalMatrix<ValueType>::CoarsenOperator(GlobalMatrix<ValueType> *Ac, ParallelManager *pm, const int nrow, const int ncol,
  ...
  for (int n=0; n<this->pm_->nrecv_; ++n)
    communication_async_recv(&recv_map_size[n], 1, this->pm_->recvs_[n], 0, &req_mapping[n], this->pm_->comm_);

  // Loop over neighbor ranks
  for (int n=0; n<this->pm_->nsend_; ++n) {

    send_ghost_map[n] = new int[this->pm_->send_offset_index_[n+1] - this->pm_->send_offset_index_[n]];

    G.ExtractCoarseMapping(this->pm_->send_offset_index_[n], this->pm_->send_offset_index_[n+1],
                           this->pm_->boundary_index_, nrow, &send_map_size[n], send_ghost_map[n]);

    // Send sizes
    communication_async_send(&send_map_size[n], 1, this->pm_->sends_[n], 0,
                             &req_mapping[this->pm_->nrecv_+n], this->pm_->comm_);
  ...
  for (int n=0; n<this->pm_->nrecv_; ++n) {
    ...
    communication_async_recv(recv_ghost_map[n], recv_map_size[n], this->pm_->recvs_[n], 0, &req_mapping[n], this->pm_->comm_);

  }

  // Send mapping pairs
  for (int n=0; n<this->pm_->nsend_; ++n) {
    communication_async_send(send_ghost_map[n], send_map_size[n], this->pm_->sends_[n], 0, 
                             &req_mapping[this->pm_->nrecv_+n], this->pm_->comm_);
  }
  ...
  for (int n=0; n<this->pm_->nsend_; ++n) {
    ...
    G.ExtractCoarseBoundary(this->pm_->send_offset_index_[n], this->pm_->send_offset_index_[n+1],
                            this->pm_->boundary_index_, nrow, &m, boundary_index);
  ...
  for (int n=0; n<this->pm_->nrecv_; ++n) {
    communication_async_recv(&recv_offset_index[n+1], 1, this->pm_->recvs_[n], 0, &req_offsets[n], this->pm_->comm_);
  }

  for (int n=0; n<this->pm_->nsend_; ++n) {
    communication_async_send(&send_offset_index[n+1], 1, this->pm_->sends_[n], 0,
                             &req_offsets[this->pm_->nrecv_+n], this->pm_->comm_);
  }
  ...
  if (this->is_accel()) {
    ...
    this->matrix_ghost_.CoarsenOperator(&tmp_ghost, nrow, this->pm_->GetNumReceivers(), G_ghost, Gsize, rG, rGsize);
  ...
  Ac->SetDataPtrCSR(&Ac_interior_row_offset, &Ac_interior_col, &Ac_interior_val,
                    &Ac_ghost_row_offset, &Ac_ghost_col, &Ac_ghost_val,
                    "", nnzc, nnzg);
