...
bool GPUAcceleratorMatrixCSR<ValueType>::Permute( const BaseVector<int> &permutation){
  ...
  if (this->get_nnz() > 0) {
    ...
    kernel_permute_rows<<<GridSize, BlockSize>>> (this->get_nrow(), this->mat_.row_offset, d_nnzPerm,
                                                  this->mat_.col, CUDAPtr(this->mat_.val), CUDAPtr(cast_perm->vec_),
                                                  d_nnzr, d_offset, CUDAPtr(d_data));
    CHECK_CUDA_ERROR(__FILE__,__LINE__);
    ...
    if (maxnnzrow > 64)
      kernel_permute_cols_fallback<<<GridSize, BlockSize>>>(this->get_nrow(), this->mat_.row_offset,
                                                            ...
                                                            CUDAPtr(d_data), this->mat_.col, CUDAPtr(this->mat_.val));
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<double> >::Apply(const BaseVector<std::complex<double> > &in,
  ...
  if (this->get_nnz() > 0) {
    ...
    stat_t = cusparseZcsrmv(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                            CUSPARSE_OPERATION_NON_TRANSPOSE,
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<float> >::Apply(const BaseVector<std::complex<float> > &in,
  ...
  if (this->get_nnz() > 0) {
    ...
    stat_t = cusparseCcsrmv(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                            CUSPARSE_OPERATION_NON_TRANSPOSE,
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<double> >::ApplyAdd(const BaseVector<std::complex<double> > &in,
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<float> >::ApplyAdd(const BaseVector<std::complex<float> > &in,
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<double> >::ILU0Factorize(void) {
  ...
  if (this->get_nnz() > 0) {
    ...
    stat_t = cusparseZcsrsv_analysis(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                                     CUSPARSE_OPERATION_NON_TRANSPOSE,
                                     this->get_nrow(), this->get_nnz(),
                                     this->mat_descr_,
                                     (cuDoubleComplex*)this->mat_.val, this->mat_.row_offset, this->mat_.col,
    ...
    stat_t = cusparseZcsrilu0(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                              CUSPARSE_OPERATION_NON_TRANSPOSE,
                              this->get_nrow(),
                              this->mat_descr_,
                              (cuDoubleComplex*)this->mat_.val, this->mat_.row_offset, this->mat_.col,
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<float> >::ILU0Factorize(void) {
  ...
  if (this->get_nnz() > 0) {
    ...
    stat_t = cusparseCcsrsv_analysis(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                                     CUSPARSE_OPERATION_NON_TRANSPOSE,
                                     this->get_nrow(), this->get_nnz(),
                                     this->mat_descr_,
                                     (cuFloatComplex*)this->mat_.val, this->mat_.row_offset, this->mat_.col,
    ...
    stat_t = cusparseCcsrilu0(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                              CUSPARSE_OPERATION_NON_TRANSPOSE,
                              this->get_nrow(),
                              this->mat_descr_,
                              (cuFloatComplex*)this->mat_.val, this->mat_.row_offset, this->mat_.col,
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<double> >::ICFactorize(BaseVector<std::complex<double> > *inv_diag) {
  ...
  if (this->get_nnz() > 0) {
    ...
    stat_t = cusparseZcsric0(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                             CUSPARSE_OPERATION_NON_TRANSPOSE,
                             this->get_nrow(),
                             this->mat_descr_,
                             (cuDoubleComplex*)this->mat_.val, this->mat_.row_offset, this->mat_.col,
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<float> >::ICFactorize(BaseVector<std::complex<float> > *inv_diag) {
  ...
  if (this->get_nnz() > 0) {
    ...
    stat_t = cusparseCcsric0(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                             CUSPARSE_OPERATION_NON_TRANSPOSE,
                             this->get_nrow(),
                             this->mat_descr_,
                             (cuFloatComplex*)this->mat_.val, this->mat_.row_offset, this->mat_.col,
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<double> >::LUAnalyse(void) {
    ...
    assert(this->get_ncol() == this->get_nrow());
    assert(this->tmp_vec_ == NULL);
    this->tmp_vec_ = new GPUAcceleratorVector<std::complex<double> >(this->local_backend_);
    assert(this->tmp_vec_ != NULL);
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<float> >::LUAnalyse(void) {
    ...
    assert(this->get_ncol() == this->get_nrow());
    assert(this->tmp_vec_ == NULL);
    this->tmp_vec_ = new GPUAcceleratorVector<std::complex<float> >(this->local_backend_);
    assert(this->tmp_vec_ != NULL);
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<float> >::LUSolve(const BaseVector<std::complex<float> > &in,
                                                            BaseVector<std::complex<float> > *out) const {
  ...
  if (this->get_nnz() > 0) {
    ...
    // Solve L
    stat_t = cusparseCcsrsv_solve(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                                  CUSPARSE_OPERATION_NON_TRANSPOSE,
                                  this->get_nrow(),
                                  (cuFloatComplex*)&one,
                                  this->L_mat_descr_,
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<double> >::LUSolve(const BaseVector<std::complex<double> > &in,
                                                             BaseVector<std::complex<double> > *out) const {
  ...
  if (this->get_nnz() > 0) {
    ...
    // Solve L
    stat_t = cusparseZcsrsv_solve(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                                  CUSPARSE_OPERATION_NON_TRANSPOSE,
                                  this->get_nrow(),
                                  (cuDoubleComplex*)&one,
                                  this->L_mat_descr_,
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<double> >::LLAnalyse(void) {
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<float> >::LLAnalyse(void) {
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<double> >::LLSolve(const BaseVector<std::complex<double> > &in,
                                                             BaseVector<std::complex<double> > *out) const {
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<float> >::LLSolve(const BaseVector<std::complex<float> > &in,
                                                            BaseVector<std::complex<float> > *out) const {
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<double> >::LAnalyse(const bool diag_unit) {
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<float> >::LAnalyse(const bool diag_unit) {
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<double> >::UAnalyse(const bool diag_unit) {
...
template <>
void GPUAcceleratorMatrixCSR<std::complex<float> >::UAnalyse(const bool diag_unit) {
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<double> >::LSolve(const BaseVector<std::complex<double> > &in,
                                                            BaseVector<std::complex<double> > *out) const {
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<float> >::LSolve(const BaseVector<std::complex<float> > &in,
                                                           BaseVector<std::complex<float> > *out) const {
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<double> >::USolve(const BaseVector<std::complex<double> > &in,
                                                            BaseVector<std::complex<double> > *out) const {
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<float> >::USolve(const BaseVector<std::complex<float> > &in,
                                                           BaseVector<std::complex<float> > *out) const {
...
bool GPUAcceleratorMatrixCSR<ValueType>::ExtractDiagonal(BaseVector<ValueType> *vec_diag) const {
  ...
  if (this->get_nnz() > 0)  {
    ...
    kernel_csr_extract_diag<<<GridSize, BlockSize>>>(nrow, this->mat_.row_offset, this->mat_.col,
                                                     CUDAPtr(this->mat_.val), CUDAPtr(cast_vec_diag->vec_));
    CHECK_CUDA_ERROR(__FILE__, __LINE__);
...
bool GPUAcceleratorMatrixCSR<ValueType>::ExtractInverseDiagonal(BaseVector<ValueType> *vec_inv_diag) const {
  ...
  if (this->get_nnz() > 0) {
    ...
    kernel_csr_extract_inv_diag<<<GridSize, BlockSize>>>(nrow, this->mat_.row_offset, this->mat_.col,
                                                         CUDAPtr(this->mat_.val), CUDAPtr(cast_vec_inv_diag->vec_));
    CHECK_CUDA_ERROR(__FILE__, __LINE__);
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<double> >::Scale(const std::complex<double> alpha) {
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<float> >::Scale(const std::complex<float> alpha) {
...
bool GPUAcceleratorMatrixCSR<ValueType>::ScaleDiagonal(const ValueType alpha) {
  ...
  if (this->get_nnz() > 0) {
    ...
    kernel_csr_scale_diagonal<<<GridSize, BlockSize>>> (nrow, this->mat_.row_offset, this->mat_.col,
                                                        CUDAVal(alpha), CUDAPtr(this->mat_.val));
    CHECK_CUDA_ERROR(__FILE__, __LINE__);
...
bool GPUAcceleratorMatrixCSR<ValueType>::AddScalar(const ValueType alpha) {
  ...
  if (this->get_nnz() > 0) {
    ...
    kernel_buffer_addscalar<<<GridSize, BlockSize>>>(nnz, CUDAVal(alpha), CUDAPtr(this->mat_.val));
    CHECK_CUDA_ERROR(__FILE__, __LINE__);
...
bool GPUAcceleratorMatrixCSR<ValueType>::DiagonalMatrixMultR(const BaseVector<ValueType> &diag) {
  ...
  if (this->get_nnz() > 0) {
    ...
    kernel_csr_diagmatmult_r<<<GridSize, BlockSize>>>(nrow, this->mat_.row_offset, this->mat_.col,
                                                      CUDAPtr(cast_diag->vec_), CUDAPtr(this->mat_.val));
    CHECK_CUDA_ERROR(__FILE__, __LINE__);
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<double> >::MatrixAdd(const BaseMatrix<std::complex<double> > &mat,
  ...
  if (this->get_nnz() > 0) {
    ...
    const GPUAcceleratorMatrixCSR<std::complex<double> > *cast_mat = dynamic_cast<const GPUAcceleratorMatrixCSR<std::complex<double> >*> (&mat);
    assert(cast_mat != NULL);
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<float> >::MatrixAdd(const BaseMatrix<std::complex<float> > &mat,
  ...
  if (this->get_nnz() > 0) {
    ...
    const GPUAcceleratorMatrixCSR<std::complex<float> > *cast_mat = dynamic_cast<const GPUAcceleratorMatrixCSR<std::complex<float> >*> (&mat);
    assert(cast_mat != NULL);
...
bool GPUAcceleratorMatrixCSR<ValueType>::Compress(const double drop_off) {
  ...
  if (this->get_nnz() > 0) {
    ...
    kernel_csr_compress_count_nrow<<<GridSize, BlockSize>>>(this->mat_.row_offset, this->mat_.col,
                                                            CUDAPtr(this->mat_.val), this->get_nrow(),
                                                            drop_off, row_offset);
    CHECK_CUDA_ERROR(__FILE__, __LINE__);      
    ...
    kernel_csr_compress_copy<<<GridSize, BlockSize>>>(tmp.mat_.row_offset, tmp.mat_.col, CUDAPtr(tmp.mat_.val),
                                                      tmp.get_nrow(), drop_off, this->mat_.row_offset,
                                                      this->mat_.col, CUDAPtr(this->mat_.val));
    CHECK_CUDA_ERROR(__FILE__, __LINE__);      
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<double> >::Transpose(void) {

  if (this->get_nnz() > 0) {

    GPUAcceleratorMatrixCSR<std::complex<double> > tmp(this->local_backend_);
    ...
    stat_t = cusparseZcsr2csc(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                              tmp.get_nrow(), tmp.get_ncol(), tmp.get_nnz(),
...
template <>
bool GPUAcceleratorMatrixCSR<std::complex<float> >::Transpose(void) {

  if (this->get_nnz() > 0) {

    GPUAcceleratorMatrixCSR<std::complex<float> > tmp(this->local_backend_);
    ...
    stat_t = cusparseCcsr2csc(CUSPARSE_HANDLE(this->local_backend_.GPU_cusparse_handle),
                              tmp.get_nrow(), tmp.get_ncol(), tmp.get_nnz(),
...
bool GPUAcceleratorMatrixCSR<ValueType>::ReplaceColumnVector(const int idx, const BaseVector<ValueType> &vec) {
  ...
  if (this->get_nnz() > 0) {
    ...
    kernel_csr_replace_column_vector_offset<<<GridSize, BlockSize>>>(this->mat_.row_offset, this->mat_.col, nrow,
                                                                     idx, CUDAPtr(cast_vec->vec_), row_offset);
    CHECK_CUDA_ERROR(__FILE__, __LINE__);
    ...
    kernel_csr_replace_column_vector<<<GridSize, BlockSize>>>(this->mat_.row_offset, this->mat_.col,
                                                              CUDAPtr(this->mat_.val), nrow, idx,
                                                              CUDAPtr(cast_vec->vec_), row_offset, col,
                                                              CUDAPtr(val));
    CHECK_CUDA_ERROR(__FILE__, __LINE__);
...
bool GPUAcceleratorMatrixCSR<ValueType>::ExtractRowVector(const int idx, BaseVector<ValueType> *vec) const {
  ...
  if (this->get_nnz() > 0) {
    ...
    kernel_csr_extract_row_vector<<<GridSize, BlockSize>>>(this->mat_.row_offset, this->mat_.col,
                                                           CUDAPtr(this->mat_.val), row_nnz, idx,
                                                           CUDAPtr(cast_vec->vec_));
    CHECK_CUDA_ERROR(__FILE__, __LINE__);
...
template class GPUAcceleratorMatrixCSR<std::complex<double> >;
template class GPUAcceleratorMatrixCSR<std::complex<float> >;
#endif
