#ifndef PARALUTION_GPU_COMPLEX_HPP_
#define PARALUTION_GPU_COMPLEX_HPP_
...
__device__ static __inline__ double cuda_abs(const double val) { return abs(val); }
__device__ static __inline__ float cuda_abs(const float val) { return abs(val); }
__device__ static __inline__ double cuda_abs(const cuDoubleComplex &val) { return cuCabs(val); }
__device__ static __inline__ float cuda_abs(const cuFloatComplex &val) { return cuCabsf(val); }
...
__device__ static __inline__ void atomic_add_cuda(float *address, float val) { atomicAdd(address, val); }
__device__ static __inline__ void atomic_add_cuda(double *address, double val) { unsigned long long int *address_as_ull = (unsigned long long int*) address; unsigned long long int old = *address_as_ull, assumed; do { assumed = old; old = atomicCAS(address_as_ull, assumed, __double_as_longlong(val + __longlong_as_double(assumed))); }while (assumed != old); }
__device__ static __inline__ void atomic_add_cuda(cuFloatComplex *address, cuFloatComplex val) { atomic_add_cuda((float*) address, val.x); atomic_add_cuda((float*) address+1, val.y); }
__device__ static __inline__ void atomic_add_cuda(cuDoubleComplex *address, cuDoubleComplex val) { atomic_add_cuda((double*) address, val.x); atomic_add_cuda((double*) address+1, val.y); }

}

#endif // PARALUTION_GPU_COMPLEX_HPP_
